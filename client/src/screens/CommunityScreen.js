...
export default function CommunityScreen({ onBack, session, onNavigate }) {
  ...
  // Sync selectedPost when posts update (to show new comments immediately)
  useEffect(() => {
    if (selectedPost) {
      ...
      if (data) {
        ...
        if (data.length > 0) {
          ...
          if (data[0].comments && data[0].comments.length > 0) {
            console.log("Debug Comment Raw:", data[0].comments[0]);
          }
        }
        ...
        const formatted = data.map(post => ({
          ...
          comments: (post.comments || []).map(comment => {
            const mapped = {
              ...comment,
              text: comment.content, // Map content (DB) to text (UI)
  ...
  const deleteCommentFromPost = async (postId, commentId) => {
    try {
      const { error } = await supabase
        .from('comments')
        .delete()
        .match({ id: commentId, user_id: session.user.id }); // Ensure ownership
